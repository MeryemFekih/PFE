...
export default function MeetingInterface() {
  ...
  const CHANNEL_NAME = params?.roomId as string;  const TOKEN = null
    ...
    <div className="flex h-screen bg-gray-100 ">
      <div className="flex-1/2 flex-col p-3">
        ...
        <div className=" bg-white p-4 mt-4 rounded-3xl flex flex-col items-center space-y-4">
          <div className="flex space-x-4 h-93">
            <video ref={localVideoRef} autoPlay playsInline className="rounded-lg bg-gray-200" />
          </div>
          <div className="flex space-x-4">
            <button onClick={toggleMic} className="bg-gray-400 text-white p-3 rounded-full flex items-center">
            ...
            <button onClick={toggleRecording} className={`p-3 rounded-full  ${isRecording ? 'bg-red-600 text-white' : 'bg-gray-400 text-white'}`}>
                <FontAwesomeIcon icon={faLaptop} className="w-6 h-6" />
            </button>
            <button onClick={leaveRoom} className="w-12 h-12 flex items-center justify-center bg-red-600 rounded-full hover:bg-red-700 transition">
            ...
            <div className="flex flex-wrap space-x-4">
              {participants.map((p, i) => (
                <div key={i} className="relative flex flex-col items-center space-y-2">
                  {p.cameraOn ? (
                    <video ref={remoteVideoRef} autoPlay playsInline className="rounded-lg w-48 h-36 bg-gray-300" />
                  ) : (
                    <div className=" rounded-lg w-48 h-36 bg-gray-300 flex flex-col justify-center items-center">                      {p.photoURL ? (
                        <img
                          src={p.photoURL} alt={p} className="w-16 h-16 rounded-full object-cover" />
                      ) : (
...
<div className="space-y-3">
  {messages.length > 0 ? (
    messages.map(msg => {
      ...
      const formattedTime = format(messageTime, 'hh:mm a'); // Format as HH:mm AM/PM (You can customize this format)

      return (
        <div key={msg.id} className={msg.sender === 'You' ? 'text-right' : ''}>
          <p className="text-sm  text-gray-500">{msg.sender} <span className="text-xs justify-end text-gray-400">{formattedTime}</span>
          </p>
        ...
        {participants.length > 0 ? (
          ...
          {participants.map((p, i) => (
            <li key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100">
              {p.photoURL ? (
                <img
                  src={p.photoURL} alt={p} className="w-8 h-8 rounded-full object-cover"/>
              ) : (<FontAwesomeIcon icon={faUserCircle} className="text-2xl text-gray-500" />)}<span>{p}</span>
            </li>
          ))}
        </ul>
        ) : (
