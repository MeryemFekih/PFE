...
export default function FacultyFeedPage({ posts, session }: FacultyFeedPageProps) {
  ...
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);
    ...
    const isPublicUser = session?.user?.role === 'PUBLIC';  const [suggestedPosts, setSuggestedPosts] = useState<{ events: Post[]; followedPosts: Post[] }>({
    events: [],
    followedPosts: [],
  });
  ...
  useEffect(() => {
    const loadSuggestions = async () => {
      ...
      try {
        ...
        setSuggestedPosts({ events: [], followedPosts: [] });
      } finally {
        setIsLoading(false);
      }
    };
