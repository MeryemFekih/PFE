import { PUBLIC_BACKEND_URL as BACKEND_URL } from "@/lib/constants";
...
export const fetSessionInfo = async () => {
  const res = await fetch('/api/session');
  if (!res.ok) throw new Error('Session not found');
  return res.json();
};

export const fetchTasks = async (userId: string, accessToken: string) => {
...
export const fetchEvents = async (userId: string, accessToken: string) => {
  const res = await fetch(`${BACKEND_URL}/events/${userId}`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
    },
...
export async function sendReminderSMS({
  ...
  try {
    await fetch(`${BACKEND_URL}/notifications/sms`, {
      method: 'POST',
...
export const deleteEvent = async (
  ...
  const res = await fetch(`${BACKEND_URL}/events/${userId}/${eventId}`, {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
